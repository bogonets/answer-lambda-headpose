...
def predict_head_pose(image, ad, img_size, img_w, img_h, model, net, time_detection=0, time_network=0, time_plot=0, image_out=True):
    ...
    # detected = detector.detect_faces(draw_img)
    ...
    if detected.shape[2] > 0:
        for i in range(0, detected.shape[2]):
            ...
            if confidence > 0.5:
                ...
                faces[i, :, :, :] = cv2.resize(
                    draw_img[yw1:yw2 + 1, xw1:xw2 + 1, :], (img_size, img_size))
                faces[i, :, :, :] = cv2.normalize(
                    faces[i, :, :, :], None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
                ...
                if image_out:
                    img = draw_axis(
                        draw_img[yw1:yw2 + 1, xw1:xw2 + 1, :], p_result[0][0], p_result[0][1], p_result[0][2])

                    draw_img[yw1:yw2 + 1, xw1:xw2 + 1, :] = img
...
def main():
    ...
    while True:
        ...
        if img_idx == 1 or img_idx % skip_frame == 0:
            ...
            input_img = draw_results_ssd(detected, input_img, faces, ad, img_size,
                                         img_w, img_h, model, time_detection, time_network, time_plot)
